...
namespace Brewsic
{
	public class Sample
	{
		...
		private void FixLooping(int maximumSampleGrowth, Action<string> output)
		{
			...
			output("Sample: " + Name);
			output("Loopstart: " + LoopStart + ", LoopEnd: " + LoopEnd + ", Length: " + Data.Length);
			...
			output("Endalign: " + endAlign);
			if (endAlign != 0) // If endAlign == 0, the loop is already aligned
			{
				if (true || addedBytes > maximumSampleGrowth)
				{
					ResampleLoop(16 - (loopSize % 16));
					output("NEW Loopstart: " + LoopStart + ", LoopEnd: " + LoopEnd + ", Length: " + Data.Length);
				}
				else
				{
		...
		private void AlignLoop(Action<string> output)
		{
			if (LoopStart % 16 != 0)
			{
				output("invalid loop start");
				// Pad the beginning
				...
				LoopEnd += (uint)samplesToAdd;
				output("NEWER Loopstart: " + LoopStart + ", LoopEnd: " + LoopEnd + ", Length: " + Data.Length);
			}
			if (LoopEnd == 0 && Data.Length % 16 != 0)
			{
		...
		private void Downsample(int newLength)
		{
			...
			for (var t = 0; t < data.Length; t++)
			{
				...
				var ratio = sampleIndex % 1; // Find target distance between the two samples
			...
			LoopStart = (uint)Math.Round(LoopStart / resizeFactor);
			LoopEnd = (uint)Math.Round(LoopEnd / resizeFactor);
		...
		private void ResampleLoop(UInt32 samplesToAdd)
		{
			...
			var resizeFactor = (double)loopSize / newLoopSize;

			var newLength = (uint)Math.Round(Data.Length / resizeFactor);
			...
			Data = data;
			_newLength = (int)newLength;
		}

		private void Resample(double fromRate, double toRate, Action<string> output)
		{
			var bytes = new byte[_originalData.Length * 2];
			for (var n = 0; n < _originalData.Length; n++)
			{
				var sampleBytes = BitConverter.GetBytes(_originalData[n]);
				bytes[n * 2] = sampleBytes[0];
				bytes[n * 2 + 1] = sampleBytes[1];
			}

			var ms = new MemoryStream(bytes);
			var rs = new RawSourceWaveStream(ms, new WaveFormat((int)Math.Round(fromRate), 16, 1));
			using (var resampler = new MediaFoundationResampler(rs, new WaveFormat((int)Math.Round(toRate), 1)))
			{
				var samples = new List<short>();
				var sampleProvider = resampler.ToSampleProvider().ToMono();
				var buffer = new float[1000];
				var readSamples = 0;
				while ((readSamples = sampleProvider.Read(buffer, 0, 1000)) > 0)
				{
				...
				Data = data;
				if (Data.Length != _newLength)
				{
					if (Data.Length - _newLength <= 1 || true)
					{
						Data = new short[_newLength];
						//Array.Copy(data, 0, Data, 0, _newLength);
						Array.Copy(data, 0, Data, 0, Math.Min(_newLength, data.Length));
					}
					else throw new Exception("resample error. New length is " + Data.Length + " - projected length was " + _newLength);
				}
			}
		}
		...
		public byte[] GetBrr(int maximumSampleGrowth = 500, int maxSampleSize = 0, Action<string> output = null, CancellationToken? token = null)
		{
			var brrData = new List<byte>();
			if (Data == null || Data.Length == 0) return brrData.ToArray();


			_originalData = new Int16[Data.Length];
			Array.Copy(Data, _originalData, Data.Length);
			var oldC5 = C5Speed;

			maxSampleSize = maxSampleSize / 9 * 16; // Convert BRR file size to sample length;
			if (maxSampleSize > 500 && Data.Length > maxSampleSize)
			{
				Downsample(maxSampleSize);
				output("sizeDownsampled " + Name + " from "+ oldC5 + " to " + C5Speed);
			}
			var nextC5 = C5Speed;
			...
			if (C5Speed != nextC5)
			{
				output("loopDownsampled " + Name + " from " + nextC5 + " to " + C5Speed);
			}

			if (C5Speed != oldC5) Resample(oldC5, C5Speed, output);
			AlignLoop(output);
			if (!StartsWithSilence(Data, 8))
			{
				output("Adding silence to " + Name + " to prevent popping");
			...
			output("Data length: " + data.Length + ", block count: " + blockCount);
			for (var blockIndex = 0; blockIndex < blockCount; blockIndex++)
			{
				if (token?.IsCancellationRequested ?? true) return null;

				var startingBlock = blockIndex == 0 || blockIndex == (LoopStart / 16);
				var block = CompressBlock(data.Skip(blockIndex * 16).Take(16).ToArray(), startingBlock, backBuffer);
